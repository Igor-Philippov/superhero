...
//  }
  ...
  private RequestSpecification prepareSecured() {
	  return prepare().auth().basic(config.getSpringSecurityUserName(), config.getSpringSecurityUserPwd()).when();
  }
  
  private RequestSpecification prepareSecured(String body) {
	  return prepare(body).auth().basic(config.getSpringSecurityUserName(), config.getSpringSecurityUserPwd()).when();
  }
}
